...
const dataEqControl = {
    ...
    bindings: [
        ...
        { binding_type: 'view', source_property_name: "('%.1s' * (#texto - ('%.24s' * #texto)))", target_property_name: '#side' },
        // h_cur at 26, h_max at 32
        { binding_type: 'view', source_property_name: "('%.5s' * (#texto - ('%.26s' * #texto)))", target_property_name: '#h_cur' },
        { binding_type: 'view', source_property_name: "('%.5s' * (#texto - ('%.32s' * #texto)))", target_property_name: '#h_max' },
        // c_cur at 38, c_max at 44
        { binding_type: 'view', source_property_name: "('%.5s' * (#texto - ('%.38s' * #texto)))", target_property_name: '#c_cur' },
        { binding_type: 'view', source_property_name: "('%.5s' * (#texto - ('%.44s' * #texto)))", target_property_name: '#c_max' },
        // l_cur at 50, l_max at 56
        { binding_type: 'view', source_property_name: "('%.5s' * (#texto - ('%.50s' * #texto)))", target_property_name: '#l_cur' },
        { binding_type: 'view', source_property_name: "('%.5s' * (#texto - ('%.56s' * #texto)))", target_property_name: '#l_max' },
        // f_cur at 62, f_max at 68
        { binding_type: 'view', source_property_name: "('%.5s' * (#texto - ('%.62s' * #texto)))", target_property_name: '#f_cur' },
        { binding_type: 'view', source_property_name: "('%.5s' * (#texto - ('%.68s' * #texto)))", target_property_name: '#f_max' },
        // o_cur at 74, o_max at 80
        { binding_type: 'view', source_property_name: "('%.5s' * (#texto - ('%.74s' * #texto)))", target_property_name: '#o_cur' },
        { binding_type: 'view', source_property_name: "('%.5s' * (#texto - ('%.80s' * #texto)))", target_property_name: '#o_max' },
    ],
};

// Slot definitions (bottom → top order in layout, right-side y increases going up)
const SLOTS = [
    { name: 'offhand', curProp: '#o_cur', maxProp: '#o_max', bg: 'textures/ui/sweepnslash/offhand_slot', yR: -26,  yL: -26  },
    { name: 'feet',    curProp: '#f_cur', maxProp: '#f_max', bg: 'textures/ui/sweepnslash/feet_slot',    yR: -50,  yL: -50  },
    { name: 'legs',    curProp: '#l_cur', maxProp: '#l_max', bg: 'textures/ui/sweepnslash/legs_slot',    yR: -74,  yL: -74  },
    { name: 'chest',   curProp: '#c_cur', maxProp: '#c_max', bg: 'textures/ui/sweepnslash/chest_slot',   yR: -98,  yL: -98  },
    { name: 'head',    curProp: '#h_cur', maxProp: '#h_max', bg: 'textures/ui/sweepnslash/head_slot',    yR: -122, yL: -122 },
] as const;
...
console.log('Generated equipment_overlay.json');
